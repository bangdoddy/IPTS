...
export function QuickModuleGrid({
  modules,
  selectedId,
  onSelect,
  onResetFilters
      ...
      {modules.map((m) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative flex flex-col items-center justify-center p-4 rounded-2xl transition-all duration-300 group overflow-hidden border-2",
              isSelected 
                ? "scale-[1.02] shadow-xl" 
                : "bg-background border-border hover:border-primary/50 hover:shadow-lg"
            )}
            style={{ 
              background: isSelected ? theme.baseGradient : undefined,
              borderColor: isSelected ? "transparent" : undefined,
              color: isSelected ? "#fff" : "inherit"
            }}
          >
            <div className={cn(
              "w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center mb-3 transition-transform duration-300 group-hover:scale-110",
              isSelected ? "bg-white/20" : "bg-muted"
            )}>
              <m.icon className={cn("w-5 h-5 sm:w-6 sm:h-6", !isSelected && "text-muted-foreground")} />
            </div>
            <span className="text-xs sm:text-sm font-bold tracking-tight uppercase">{m.id}</span>
            <span className={cn("text-[10px] mt-1 font-medium", isSelected ? "text-white/80" : "text-muted-foreground")}>
              {m.title}
            </span>
            {isSelected && (

...
import { FinishOverviewBarChart } from "./qcc/FinishOverviewBarChart";
import { SsDashboardFilterBar, SS_DEFAULT_YEAR } from "./ss/SsDashboardFilterBar";
...
/** Monthly Trend, Department View — banyak kategori */
    ...
    (async () => {
      try {
        ...
        if (USE_DUMMY_PROJECT_STATUS) {
          ...
          if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART_SITE)) {
            mockResult = resolveSsDashboardMock("site", params.body ?? {});
          } else if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART_TOTAL)) {
            mockResult = resolveSsDashboardMock("total", params.body ?? {});
          } else if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART)) {
            mockResult = resolveSsDashboardMock("collected", params.body ?? {});
          }

          if (mockResult != null) {
            await new Promise((r) => setTimeout(r, 300));
            if (alive) {
              const mapped = params.mapData
                ? params.mapData(mockResult)
                : (mockResult as T);
              setData(mapped);
            }
            return;
...
export function PlanActualChartCard(props: {
  ...
  actualColor?: string;
  hideFilters?: boolean;
  /** Saat hideFilters: tampilkan dropdown bulan saja (jobsite filter di dalam card) */
  ...
  const chartBottom = props.chartMarginBottom ?? 120;
  ...
  const yTickFontSize = props.yTickFontSize ?? 11;
  return (
    <Card className={cn("transition-all duration-300", props.hideCardOutline ? "bg-transparent shadow-none border-none" : "shadow-md border border-border/50 bg-white rounded-2xl overflow-hidden")}>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className={cn("flex flex-col", props.centeredTitle ? "items-center justify-center text-center" : "")}>
              ...
              <p
                className={cn(
                  "mt-2",
                  props.subtitleClassName ?? "text-sm text-muted-foreground font-medium opacity-80"
                )}
                style={
                  props.titleIconSize
      ...
      <CardContent className={cn("p-6 pt-2 relative", props.hideCardOutline ? "px-0" : "")}>
        {!props.data || props.data.length === 0 ? (
          <div className="w-full h-[500px] sm:h-[550px] flex flex-col items-center justify-center text-muted-foreground bg-slate-50/50 rounded-xl border border-dashed border-border/60">
              ...
              >
                  ...
                  content={(tip: any) => {
                    ...
                    return (
                      <TooltipShell
                        title={String(label ?? "")}
                        lines={[
                          { label: "Target (Plan)", value: d.Plan, color: planColor },
                          { label: "Actual Collected", value: d.Actual, color: actualColor },
                        ]}
                ...
                <Legend
                  ...
                  wrapperStyle={{
                    paddingTop: "0px",
                    paddingBottom: "30px",
                    fontSize: `${props.legendFontSize ?? 11}px`,
                    fontWeight: "600",
                  }}
                />

                <Bar dataKey="Plan" name="Target Plan" fill={`url(#barGradientPlan_${gradientKey})`} radius={[6, 6, 0, 0]} maxBarSize={barSize} animationDuration={1000}>
                  <LabelList
                    dataKey="Plan"
                    position="top"
                    content={(p: any) => (
                ...
                <Bar dataKey="Actual" name="Actual Performance" fill={`url(#barGradientActual_${gradientKey})`} radius={[6, 6, 0, 0]} maxBarSize={barSize} animationDuration={1200}>
                  <LabelList
                    dataKey="Actual"
                    position="top"
                    content={(p: any) => (
      ...
      <div className="grid grid-cols-4 gap-2">
        {[1, 2, 3, 4, 5, 6, 7, 8].map((step) => {
          ...
          return (
            ...
            >
              <p className="text-xs text-muted-foreground mb-1">Langkah {step}</p>
...
export default function DashboardLegacy(props: DashboardLegacyProps) {
  ...
  useEffect(() => {
    ...
    fetch(url, {
      ...
      .catch(err => console.error("Fetch sites err", err));
  ...
  const ssMonthlyData = useMemo(() => {
    ...
    let rows: any[] = [];
    if (Array.isArray(ssDash.data)) {
      rows = ssDash.data;
    } else if (ssDash.data) {
      rows = ssDash.data.MonthlyData || ssDash.data.monthlyData || [];
    }

    return rows.map((r: any) => ({
      label: r.Label || r.label || "-",
      Plan: Number(r.Plan ?? r.plan ?? 0),
  ...
  const ssBreakdownData = useMemo(() => {
    ...
    const raw = ssBreakdownDash.data;
    if (Array.isArray(raw)) return [];

    const rows = raw?.BreakdownData || raw?.breakdownData || [];
    return rows.map((r: any) => ({
      department: r.Label || r.label || "-",
      Plan: Number(r.Plan ?? r.plan ?? 0),
      Actual: Number(r.Actual ?? r.actual ?? 0),
      Achievement: calcAchievement(Number(r.Plan ?? r.plan ?? 0), Number(r.Actual ?? r.actual ?? 0)),
    }));
  ...
  const ssAllMonthBreakdownData = useMemo(() => {
    ...
    const raw = ssDash.data;
    if (Array.isArray(raw)) return [];

    const rows = raw?.BreakdownData || raw?.breakdownData || [];
    return rows.map((r: any) => ({
      department: r.Label || r.label || r.orgUnit || "-",
      Plan: Number(r.Plan ?? r.plan ?? r.target ?? 0),
      Actual: Number(r.Actual ?? r.actual ?? 0),
      Achievement: calcAchievement(
        Number(r.Plan ?? r.plan ?? r.target ?? 0),
        Number(r.Actual ?? r.actual ?? 0)
      ),
    }));
  }, [selectedSiteSS, ssDash.data]);
  ...
  const ssChartBound = useSsChartBound({
    rows: ssChartLevel === "org" ? (ssDash.rows || []) : [],
    filter: ssFilter,
    mergedDivisions,
    mergedDepartments,
    mergedSections,
  ...
  const ssChartData = useMemo(() => {
    if (ssChartLevel === "site") {
      const rows = selectedSiteSS === "all" ? (ssDash.data || []) : [];
  ...
  return (
    <div className="w-full max-w-none py-6 sm:py-10">
      ...
      {/* Quick Filter atas — selalu tampil saat landing; sinkron dengan chart & monitoring */}
      ...
      {selectedChartModule === "ss" && (
        <div className="space-y-8 mt-0">
          ...
          <FinishOverviewBarChart
            ...
            target={ssTotalReq.data?.totalTarget ?? 0}
            actual={ssTotalReq.data?.totalActual ?? 0}
              ...
              <CardContent className="p-0">
                <div className="grid grid-cols-2 items-stretch divide-x divide-border/30 border-t border-border/30">
                  ...
                  <div className="p-2">
                    <PlanActualChartCard
                      title="Monthly Trend"
                      subtitle={`${selectedSiteSS} Performance Trend — All Months (${ssDashboardYear})`}
                      {...SS_CHART_COLORS}
                      {...SS_CHART_BAR_DENSE}
                      data={ssMonthlyData.map(d => ({
                        department: d.label,
                        Plan: d.Plan,
                        Actual: d.Actual,
                        Achievement: calcAchievement(d.Plan, d.Actual)
                      }))}
                      hideFilters={true}
                      showPaging={false}
                      yDomain={[0, Math.max(...ssMonthlyData.map(r => Math.max(r.Plan || 0, r.Actual || 0)), 10) * 1.2]}
                      centeredTitle={false}
                ...
                {ssBreakdownMonth !== "all" && (
                  <div className="border-t border-border/30 p-2">
                    <PlanActualChartCard
                      ...
                      yDomain={[
                        0,
                        Math.max(...ssBreakdownData.map((r) => r.Actual || 0), 10) * 1.2,
                      ]}
                      centeredTitle={false}
                      hideCardOutline={true}
                    />

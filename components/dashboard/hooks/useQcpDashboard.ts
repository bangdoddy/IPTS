...
export interface QcpDashboardSummary {
    site: string;
    target: number;
    active: number;
    finish: number;
...
export interface QcpDashboardMonthly {
    month: number;
    monthName: string;
    target: number;
    finish: number;
...
export interface QcpDashboardBreakdown {
    orgUnit: string;
    target: number;
    finish: number;
}

export interface QcpDashboardResponse {
    summaryData: QcpDashboardSummary[];
    monthlyData: QcpDashboardMonthly[];
    breakdownData: QcpDashboardBreakdown[];
}
...
export function useQcpDashboard(params: {
    ...
    const { data, loading } = usePostJson<any>({
        ...
        url: API.QCP_DASHBOARD,
        body,
        token,
        onError: (status, payload) => console.error("QCP Dashboard API error:", status, payload)
    });

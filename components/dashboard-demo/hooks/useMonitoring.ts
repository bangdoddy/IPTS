...
export function useMonitoring(params: {
  ...
  const [selectedModule, setSelectedModule] = useState<ModuleId | null>("ss");
  ...
  const getModuleStats = useCallback(
    (moduleId: ModuleId) => {
      ...
      return {
        submitted: isQFamily(moduleId) ? byStatusNorm("registered") + byStatusNorm("submitted") : byStatusExact("registered"),
        inProgress: isQFamily(moduleId) ? byStatusNorm("in-progress") : byStatusExact("in-progress"),
        approved: isQFamily(moduleId) ? byStatusNorm("finished") : byStatusExact("finished"),
        rejected: 0,
        
        // Legacy fields for SS or other uses
...
export function useSsChartBound(params: {
  ...
  const { rows, filter, mergedSites, mergedDivisions, mergedDepartments, mergedSections, orgFallback } = params;

  return useMemo(() => {
    const siteSel = String(filter.site ?? "").trim();
    ...
    const level: "site" | "division" | "department" | "section" =
      !siteSel ? "site" : !divisionSel ? "division" : !deptSel ? "department" : "section";

    const pageSize = 5;
    const labelsSource = level === "site" ? mergedSites : mergedDivisions;
    const totalPages = Math.max(1, Math.ceil(labelsSource.length / pageSize));
    const hasPrevPage = filter.pageIndex > 0;
    const hasNextPage = (filter.pageIndex + 1) * pageSize < labelsSource.length;
    ...
    const data: PlanActualRow[] = labels.map((label) => {
      // For demo, if label is ADMO/JAHO/MACO, it might be in department field of ssChart
      const found = rows.find(
        (r) =>
          (level === "site" && (r as any).department === label) || // Site is in department field in mock
      ...
      return {
        department: label,
        Plan: found ? Number(found.planSS ?? (found as any).Plan ?? found.plan ?? 0) : 0,
        Actual: actual,
        Achievement: 0,
      };
    });

...
export function useApiPlanActualChartBound(params: {
  ...
  return useMemo(() => {
    ...
    if (hasApiRows) {
      const mapped = mapProjectChartRowsToPlanActual(rows);
      // For demo: if data exists, it defines its own labels
      return {
        data: mapped,
        filteredDepartments,
        filteredSections,
        hasPrevPage: false,
        hasNextPage: false,
        showPaging: false, // Disable paging for static demo data
        totalPages: 1,
      };
    }

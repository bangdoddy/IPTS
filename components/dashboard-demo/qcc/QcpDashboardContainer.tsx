...
import { useQcpDashboard } from "../hooks/useQcpDashboard";
import { QcpSummaryCharts } from "./QcpSummaryCharts";
import { QcpSiteDetailsCharts } from "./QcpSiteDetailsCharts";

export function QcpDashboardContainer() {
    const [selectedYear, setSelectedYear] = useState<number>(new Date().getFullYear());
    const [selectedSite, setSelectedSite] = useState<string | null>(null);

    const { summary, monthly, breakdown, loading } = useQcpDashboard({
        year: selectedYear,
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/50 p-4 rounded-2xl border border-border/50 backdrop-blur-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight">QCP Performance Dashboard</h2>
                        <p className="text-sm text-muted-foreground font-medium">Monitoring Target vs Actual Achievements (QCP Project)</p>
                    </div>
                </div>

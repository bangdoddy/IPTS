...
export function ApprovalQcp() {
  const leaderNrp = useMemo(() => getLoginNrp(), []);
  const [rows, setRows] = useState<ApprovalQcpRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const [detail, setDetail] = useState<ApprovalQcpRow | null>(null);
  ...
  const fetchList = useCallback(async () => {
    ...
    setError("");
    try {
      const resp = await fetch((API as any).QCP_APPROVAL_LIST, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ LeaderNrp: String(leaderNrp) }),
      });
      const json = (await resp.json().catch(() => ({}))) as ApiResponse<ApprovalQcpRow[]>;
      const code = Number(json?.responseCode ?? resp.status);
      if (code !== 200) throw new Error(json?.message ?? "Gagal load approval QCP");
      setRows(Array.isArray(json?.data) ? json.data : []);
    } catch (e: any) {
      setError(e?.message || "Gagal load approval QCP");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      ...
      const step = Number(detail.step ?? 0);
      const resp = await fetch((API as any).QCP_STEP_REJECT, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ItemKey: detail.itemKey,
      ...
      <Card className="shadow-lg border-0 bg-card">
        <CardHeader className="p-4 sm:p-6 pb-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <CardTitle className="text-lg sm:text-xl">Approval QCP</CardTitle>

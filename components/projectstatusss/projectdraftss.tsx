...
/* =====================================================
   Types
   ===================================================== */
type ProjectStatus = "Review By Leader 1" | "Review By Leader" | "Review by BPI" | "Complete" | "Close" | "Reject by leader" | "Draft";
...
/** Historical SS tab: hanya tampilkan ide Draft */
const HISTORICAL_SS_STATUS = "Draft";
...
export function SSDraft() {
    ...
    const requestBodyList = useMemo(
        ...
        [currentPage, startDate, endDate, keywordDebounced, userNrp]
    );

    const list = useSuggestionList({ suggestionListUrl, requestBody: requestBodyList, pageSize: ITEMS_PER_PAGE });
    ...
    const goNext = useCallback(() => setCurrentPage((p) => Math.min(list.totalPages, p + 1)), [list.totalPages]);

    // Extract unique jobsites from site field (falling back to itemKey split)
    const jobsiteOptions = useMemo(() => {
        const set = new Set<string>();
        (list.projects || []).forEach((p) => {
            const jobsite = p.site || String(p.itemKey || "").split("/")[0];
    ...
    // Filtered projects by jobsite
    const filteredProjects = useMemo(() => {
        const completeOnly = list.projects.filter(
            (p) => normalizeStatus(p.status) === HISTORICAL_SS_STATUS
        );
        if (jobsiteFilter === "all") return completeOnly;
        return completeOnly.filter((p) => {
            const jobsite = p.site || String(p.itemKey || "").split("/")[0];
            return jobsite === jobsiteFilter;
        });
    }, [list.projects, jobsiteFilter]);
    ...
    const handleViewUploadedDoc = useCallback(() => {
        ...
        openBase64InNewTab(selectedUploadedBase64);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-semibold text-white">Project Drafts</h1>
                <p className="text-sm text-white/90 mt-1">Saved draft for Suggestion System (SS) only</p>
            </div>
            ...
            <Card className="p-2">
                        ...
                        {list.isLoading ? (
                            ...
                            filteredProjects.map((p) => (
                                <TableRow key={p.id} className="hover:bg-muted/50">
                                    ...
                                    <TableCell className="text-muted-foreground">{p.revised}</TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            onClick={() => navigate(`/module/ss?itemKey=${p.itemKey}`)}
                                        >
                ...
                <div className="flex justify-between items-center px-4 py-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                        Page {Math.min(currentPage, list.totalPages)} of {list.totalPages} • {list.totalItems} items
                    </div>

                    <Button variant="outline" size="sm" onClick={goNext} disabled={currentPage >= list.totalPages || list.isLoading}>
                        Next

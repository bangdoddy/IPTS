...
import DASHBOARD_DEMO_DATA from "./DashboardDemoData.json";



/* 
 * CREATED BY AntiGravity
 * ORCHESTRATED BY Ronald Poetra (Zaid)
 * LOCOMO SIGNATURE @2026
 */
import {
  BarChart3,
  LayoutDashboard,
...
import { API, CONFIG, API_PATH } from "../../config";
...
import { FinishOverviewBarChart } from "../dashboard/qcc/FinishOverviewBarChart";
import { SsDashboardFilterBar, SS_DEFAULT_YEAR } from "../dashboard/ss/SsDashboardFilterBar";
import { SsMonthlyAllSitesChart } from "../dashboard/ss/SsMonthlyAllSitesChart";
import { BRAND_SIDEBAR, SS_CHART_PALETTE, QCC_CHART_PALETTE } from "../dashboard/chartPalette";
...
export type DashboardLegacyProps = {
  projects: Project[];
...
const CURRENT_MONTH = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][new Date().getMonth()];
...
const MONTH_OPTIONS = MONTHS.filter((m) => m.value !== "all");

const MONTH_ORDER = ["all", ...MONTH_OPTIONS.map((m) => m.value)];

const SS_CHART_COLORS = {
  accentColor: SS_CHART_PALETTE.actual,
  planColor: SS_CHART_PALETTE.plan,
  actualColor: SS_CHART_PALETTE.actual,
} as const;

const SS_CHART_BAR_SINGLE = {
  maxBarSize: 320,
  barGap: 36,
  barCategoryGap: 0,
  chartMarginTop: 58,
  chartMarginBottom: 56,
  chartMarginLeft: 52,
  chartMarginRight: 40,
  titleClassName: "font-bold tracking-tight text-foreground",
  titleStyle: { fontSize: "clamp(1.125rem, 2.5vw, 1.75rem)", fontWeight: 700, lineHeight: 1.2 },
  titleIconClassName: "shrink-0",
  titleIconSize: 26,
  subtitleClassName: "text-xs sm:text-sm text-muted-foreground font-medium opacity-85",
  labelFontSize: 24,
  legendFontSize: 20,
  xTickFontSize: 20,
  yTickFontSize: 16,
  xTickHorizontal: true,
};

/** SS Collected — sedikit kategori (per site), lebar penuh */
const SS_CHART_BAR_SITES = {
  maxBarSize: 64,
  barGap: 10,
...
/** Monthly Trend, Department View, breakdown per bulan — banyak kategori */
const SS_CHART_BAR_DENSE = {
  maxBarSize: 48,
  barGap: 4,
...
/** Potong label sumbu X jika terlalu panjang, mis. "Coal & Fuel Transport" → "Coal & Fuel tra..." */
function truncateAxisLabel(text: string, maxLen = 18) {
...
/* =====================================================
   HOOK: chart filter
   ===================================================== */
function useChartFilter(initialYear = CURRENT_YEAR, initialMonth = CURRENT_MONTH) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const isUrl = (path: string) => url && url.includes(path);

        if (isUrl(API_PATH.SUGGESTION_LIST)) {
          const listBody = (params.body ?? {}) as Record<string, unknown>;
          const pageSize = Math.max(1, Number(listBody.pageSize ?? 10));
          const pageNumberRaw = Number(listBody.pageNumber ?? 1);
          const pageNumber = pageNumberRaw >= 1 ? pageNumberRaw - 1 : 0;
          mockResult = fetchDummySsList({
            pageNumber,
            pageSize,
            createdAtFrom: (listBody.createdAtFrom as string | null) ?? null,
            createdAtTo: (listBody.createdAtTo as string | null) ?? null,
            keyword: String(listBody.keyword ?? ""),
            status: String(listBody.status ?? ""),
          }).rows;
        }
        else if (isUrl(API_PATH.QCC_LIST)) mockResult = DASHBOARD_DEMO_DATA.qccList;
        else if (isUrl(API_PATH.QCP_LIST)) mockResult = DASHBOARD_DEMO_DATA.qcpList;
        else if (isUrl(API_PATH.QCC_CHART)) mockResult = DASHBOARD_DEMO_DATA.qccChart;
        else if (isUrl(API_PATH.QCP_CHART)) mockResult = DASHBOARD_DEMO_DATA.qcpChart;
        else if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART_SITE)) {
          mockResult = resolveSsDashboardMock("site", (params.body ?? {}) as Record<string, unknown>);
        }
        else if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART_TOTAL)) {
          mockResult = resolveSsDashboardMock("total", (params.body ?? {}) as Record<string, unknown>);
        }
        else if (isUrl(API_PATH.SUGGESTION_DASHBOARDCHART)) {
          mockResult = resolveSsDashboardMock("collected", (params.body ?? {}) as Record<string, unknown>);
        }
        else if (isUrl(API_PATH.SUGGESTION_CLASSIFICATION)) mockResult = DASHBOARD_DEMO_DATA.classification;

        if (mockResult) {
          await new Promise((resolve) => setTimeout(resolve, 500)); // simulate lag
          if (alive) {
            const processed = mockResult;
            const mapped = params.mapData ? params.mapData(processed) : (processed as T);
            setData(mapped);
            setLoading(false);
          }
          return;
...
async function postOrg<T>(
  ...
  // --- MOCK INTERCEPTOR ---
  if (url === API.ORGANIZATION_EXECUTE) {
    await new Promise(r => setTimeout(r, 500));
    return [] as any as T; // Return empty array or mock data
  }
  // --- END MOCK INTERCEPTOR ---
...
function useSsList(params: {
  ...
  const totals = useMemo(() => {
    ...
    // If API/Mock returns 0 but we have items, calculate from items (demo fallback)
    if (apiTotals.reviewByLeader === 0 && apiTotals.reviewByBPI === 0 && apiTotals.completed === 0 && items.length > 0) {
      return {
        reviewByLeader: items.filter(i => (i.status || "").toLowerCase() === "review by leader").length,
        reviewByBPI: items.filter(i => (i.status || "").toLowerCase() === "review by bpi").length,
        completed: items.filter(i => (i.status || "").toLowerCase() === "complete").length,
      };
    }
    return apiTotals;
  }, [first, items]);
    ...
    <div className="flex flex-row items-center gap-2">
      {!props.hideYear && (
        <Select value={props.year} onValueChange={props.onYear}>
          <SelectTrigger className="w-[110px] h-9 text-xs bg-muted/30 border-muted-foreground/10 hover:bg-muted/50 rounded-lg transition-colors font-semibold">
...
function RotatedTick(props: {
  ...
  const abbr = getDepartmentAbbr(fullText);
  const displayText = props.horizontal ? abbr : truncateAxisLabel(abbr, 18);
  const fs = props.fontSize ?? 12;
    ...
    <g transform={`translate(${x},${y})`}>
      ...
      {props.horizontal ? (
        <text
          ...
          dy={18}
          textAnchor="middle"
          fill={props.color ?? "var(--muted-foreground)"}
          fontSize={fs}
          fontWeight={fs >= 18 ? "700" : props.bold ? "700" : "600"}
          style={{ cursor: "help", fontFamily: "inherit" }}
        >
          {displayText}
        </text>
      ) : (
...
function TooltipShell(props: {
  ...
  return (
    <div
      className="backdrop-blur-md bg-white/90 border border-white/20 shadow-2xl rounded-xl p-3 min-w-[180px] animate-in fade-in zoom-in duration-200"
      style={{
        boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)",
      }}
    >
      <div className="text-xs font-bold text-foreground/80 mb-2 border-b border-border/50 pb-1.5 uppercase tracking-wider">
        {props.title}
      </div>
        ...
        {props.lines.map((line, i) => (
          <div key={i} className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium text-muted-foreground">{line.label}</span>
            </div>
            <span className="text-xs font-bold text-foreground">
              {typeof line.value === "number" ? line.value.toLocaleString() : line.value}
            </span>
          </div>
        ))}
      </div>
      {props.footer && (
        <div className="mt-2 pt-1.5 border-t border-border/50 text-[10px] font-semibold text-destructive animate-pulse">
          {props.footer}
        </div>
      )}
    </div>
...
function TotalChartCard(props: {
  ...
  const planColor = props.planColor ?? QCC_CHART_PALETTE.plan;
  const actualColor = props.actualColor ?? QCC_CHART_PALETTE.actual;
  return (
...
function PlanActualBarValueLabel(props: {
  x: number;
  y: number;
  width: number;
  value: unknown;
  ...
  if (!row || props.value == null || props.value === "") return null;

  const plan = Number(row.Plan) || 0;
  const actual = Number(row.Actual) || 0;
  const isUnderTarget = !props.hidePlan && plan > 0 && actual < plan;
  const fill = props.side === "actual" && isUnderTarget ? "#E11D48" : props.color;

  const maxV = Math.max(plan, actual, 1);
  const valuesSimilar =
    !props.hidePlan &&
    (Math.abs(plan - actual) / maxV < 0.12 || Math.abs(plan - actual) < 15);

  const cx = props.x + props.width / 2;
  const fs = props.fontSize ?? 15;
  const topOffset = fs + 2;
  const staggerGap = Math.round(fs * 1.25);
  const labelY = valuesSimilar
    ? props.side === "plan"
      ? props.y - topOffset
      : props.y - topOffset - staggerGap
    : props.y - topOffset;
...
export function PlanActualChartCard(props: {
  ...
  actualColor?: string;
  hideFilters?: boolean;
  showMonthFilterOnly?: boolean;

  showLevelToggle?: boolean;
  levelValue?: "org" | "site";
  onLevelChange?: (v: "org" | "site") => void;
  ...
  xTickHorizontal?: boolean;
  yTickFontSize?: number;
  chartMarginTop?: number;
}) {
  const planColor = props.planColor || SS_CHART_PALETTE.plan;
  const actualColor = props.actualColor || props.accentColor || SS_CHART_PALETTE.actual;
  const gradientKey = props.title.replace(/[^a-zA-Z0-9]/g, "_");
  const barSize = props.maxBarSize ?? 35;
  const chartTop = props.chartMarginTop ?? 52;
  const chartBottom = props.chartMarginBottom ?? 60;
  const chartLeft = props.chartMarginLeft ?? 10;
  const chartRight = props.chartMarginRight ?? 30;
  // CardTitle: text-xl (20px) / sm:text-2xl (24px) — label stays below title size
  const labelFontSize = props.labelFontSize ?? (barSize >= 80 ? 18 : 13);
  const yTickFontSize = props.yTickFontSize ?? 13;
  return (
    <Card className={cn("transition-all duration-300", props.hideCardOutline ? "bg-transparent shadow-none border-none" : "shadow-md border border-border/50 bg-white rounded-2xl overflow-hidden")}>
      <CardHeader className={cn("p-6 pb-2", props.hideCardOutline ? "px-0 pt-0" : "")}>
        <div className="mb-4">
          <div className="flex flex-col gap-5">
            <div className="flex items-center justify-between gap-2">
              <div className="w-full max-w-[180px]">
                {props.siteOptions && (
                  <Select value={props.siteValue || "all"} onValueChange={props.onSiteChange}>
                    <SelectTrigger className="h-9 text-xs bg-muted/30 border-muted-foreground/10 hover:bg-muted/50 rounded-lg transition-colors">
                      <SelectValue placeholder="All Site" />
                    </SelectTrigger>
                    <SelectContent className="rounded-xl border-border/50">
                      <SelectItem value="all" className="rounded-md">All Site</SelectItem>
                      {props.siteOptions.map((s) => {
                        if (s === "JAHO") return null;
                        const label = s === "NARO" ? "JAHO-NARO" : s;
                        return (
                          <SelectItem key={s} value={s} className="rounded-md">{label}</SelectItem>
                        );
                      })}
                    </SelectContent>
                  </Select>
              ...
              {props.hideFilters && props.showMonthFilterOnly && (
                <Select value={props.month} onValueChange={props.onMonth}>
                  <SelectTrigger className="w-[130px] h-9 text-xs font-semibold bg-muted/30 border-muted-foreground/10 rounded-lg">
                    <SelectValue placeholder="Month" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className={cn("flex flex-col", props.centeredTitle ? "items-center justify-center text-center" : "")}>
              <div className={cn("flex items-center", props.titleIconSize ? "gap-2.5" : "gap-4")}>
                <div className={cn("rounded-xl bg-primary/10 shrink-0", props.titleIconSize ? "p-2" : "p-3 sm:p-3.5")}>
                  <BarChart3
                    className={props.titleIconClassName ?? "w-5 h-5"}
                    style={{
                      color: props.accentColor,
                      ...(props.titleIconSize
                        ? { width: props.titleIconSize, height: props.titleIconSize, minWidth: props.titleIconSize, minHeight: props.titleIconSize }
                        : {}),
                    }}
                  />
                </div>
                {props.titleStyle ? (
                  <div
                    className={props.titleClassName ?? "font-bold tracking-tight text-foreground"}
                  ...
                  <CardTitle
                    className={
                      ...
                      "text-xl sm:text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70"
                    }
                  >
                    {props.title}
                  </CardTitle>
              ...
              <div
                ...
                style={
                  props.titleIconSize
                    ? { paddingLeft: props.titleIconSize + 28 }
                    : undefined
                }
              >
                <p className={props.subtitleClassName ?? "text-sm text-muted-foreground font-medium opacity-80"}>{props.subtitle}</p>
                {!props.hideFilters && (
                  <YearMonthControls 
                    year={props.year} 
                    month={props.month} 
      ...
      <CardContent className={cn("p-6 pt-2 relative", props.hideCardOutline ? "px-0" : "")}>
        {!props.data || props.data.length === 0 ? (
          <div style={{ height: props.chartHeight || 550 }} className="w-full flex flex-col items-center justify-center text-muted-foreground bg-slate-50/50 rounded-xl border border-dashed border-border/60">
            <AlertCircle className="w-10 h-10 mb-3 opacity-40 text-slate-400" />
            <p className="text-base font-semibold text-slate-600">No Data Available</p>
            <p className="text-sm opacity-70 mt-1 max-w-[60%] text-center">There is no breakdown data available for this selection.</p>
          </div>
        ) : (
          <div style={{ height: props.chartHeight || 550 }} className="w-full">
            <ResponsiveContainer width="100%" height="100%" key={`${props.title}_${props.data.length}`}>
              <BarChart
                data={props.data}
                margin={{ top: chartTop, right: chartRight, left: chartLeft, bottom: chartBottom }}
                {...(props.barGap != null ? { barGap: props.barGap } : {})}
                {...(props.barCategoryGap != null ? { barCategoryGap: props.barCategoryGap } : {})}
              >
                <defs>
                ...
                <XAxis
                  ...
                  tick={(tp: any) => {
                    ...
                    const isUnderTarget = !props.hidePlan && !!d && d.Plan > 0 && d.Actual < d.Plan;
                    ...
                    return (
                      <RotatedTick
                        ...
                        color={isUnderTarget ? "#E11D48" : undefined}
                        bold={isUnderTarget}
                        fontSize={props.xTickFontSize}
                        horizontal={props.xTickHorizontal}
                      />
                    );
                  }}
                  tickLine={false}
                ...
                <YAxis
                  domain={props.yDomain}
                  ticks={props.yTicks}
                  tick={{ fill: "var(--muted-foreground)", fontSize: yTickFontSize, fontWeight: 600 }}
                  tickLine={false}
                  axisLine={false}
                />
                  ...
                  content={(tip: any) => {
                    ...
                    return (
                      <TooltipShell
                        ...
                        footer={isUnderTarget ? "⚠️ Target Not Achieved" : undefined}
                      />
                    );
                  }}
                />
                ...
                <Legend
                  ...
                  align="right"
                  iconType="circle"
                  iconSize={props.legendFontSize != null && props.legendFontSize >= 18 ? 12 : 8}
                  wrapperStyle={{
                    paddingTop: "0px",
                    paddingBottom: "30px",
                    fontSize: `${props.legendFontSize ?? 15}px`,
                    fontWeight: "600",
                  }}
                />

                {!props.hidePlan && (
                  <Bar dataKey="Plan" name="Plan" fill={`url(#barGradientPlan_${gradientKey})`} radius={[6, 6, 0, 0]} maxBarSize={barSize} animationDuration={1000}>
                    <LabelList
                      dataKey="Plan"
                      position="top"
                      content={(p: any) => (
                        <PlanActualBarValueLabel
                          x={p.x}
                          y={p.y}
                          width={p.width}
                          value={p.value}
                ...
                <Bar dataKey="Actual" name="Actual" fill={`url(#barGradientActual_${gradientKey})`} radius={[6, 6, 0, 0]} maxBarSize={barSize} animationDuration={1200}>
                  <LabelList
                    dataKey="Actual"
                    position="top"
                    content={(p: any) => (
...
function useMonitoring(params: {
  ...
  const filteredProjects = useMemo(() => {
    if (!selectedStep) return statusFilteredProjects;
    if (isQFamily(selectedModule) && selectedStatus === "in-progress") {
      return statusFilteredProjects.filter((p) => pickNumber((p as any).step, 0) === selectedStep);
    }
    return statusFilteredProjects;
  }, [statusFilteredProjects, selectedModule, selectedStatus, selectedStep]);
  ...
  const getStatusBadge = useCallback((status: string, step: any) => {
    ...
    const map: Record<string, { label: string; cls: string }> = {
      ...
      "in-progress": {
        label: `In Progress${stepVal ? ` (Step ${stepVal}${stepStatusLabel ? ` - ${stepStatusLabel}` : ""})` : ""
          }`,
                  ...
                  if (isSS) {
                    ...
                    return (
                      <TableRow key={String(project.id ?? rowNo)}>
                        ...
                        <TableCell>
                          <TruncatedCell text={project.judulSS} />
                        </TableCell>
...
/* =====================================================
   MODULE GRID
   ===================================================== */
function moduleGridLabel(id: ModuleId, title: string, compact: boolean) {
  if (compact) return MODULE_QUICK_LABEL[id];
  return id === "crp" ? "Cost Reduction" : title;
}
      ...
      {props.modules.map((m) => {
        ...
        return (
          <Button
            ...
            className={cn(
              "h-auto flex flex-col items-center border-0 transition-all duration-300",
              compact ? "py-3 px-2 gap-1.5 min-h-[72px]" : "py-5 px-4 gap-2.5",
          ...
          >
            <Icon
              className={cn("shrink-0", compact ? "w-4 h-4" : "w-5 h-5", id === "tebp" && "text-[#E8FFFA]")}
            />
            <p
              className={cn(
                "text-center font-bold",
                compact ? "text-sm leading-none tracking-wide" : "text-xs leading-tight"
              )}
            >
              {moduleGridLabel(id, m.title, compact)}
            </p>
          </Button>
        );
      })}
...
/* =====================================================
   ...
   ===================================================== */
/**
 * TruncatedCell Component for long project titles
 */
const TruncatedCell = ({ text, limit = 60 }: { text: string; limit?: number }) => {
  const [expanded, setExpanded] = React.useState(false);
  const isLong = text && text.length > limit;

  if (!isLong) return <span className="text-sm">{text}</span>;
    ...
    <div className="max-w-[400px]">
      <span className="text-sm">
        ...
        <button
          ...
          className="ml-1 text-[10px] font-bold text-orange-600 hover:text-orange-700 transition-colors cursor-pointer"
        >
...
export default function DashboardLegacy(props: DashboardLegacyProps) {
  const projects = useMemo(() => [
    ...getSsDummySourceRows(),
    ...DASHBOARD_DEMO_DATA.qccList,
    ...DASHBOARD_DEMO_DATA.qcpList
  ] as Project[], []);
  ...
  // ----- Chart state -----
  const chartFilter = useChartFilter(CURRENT_YEAR, "all");
  const ssFilter = chartFilter;
  const qccFilter = chartFilter;
  const qcpFilter = chartFilter;

  const [ssChartLevel, setSsChartLevel] = useState<"org" | "site">("site");
  const [selectedSiteSS, setSelectedSiteSS] = useState("all");
  const [ssBreakdownMonth, setSsBreakdownMonth] = useState("all");
  const ssMonthDefaultApplied = useRef(false);
  const [ssDashboardYear, setSsDashboardYear] = useState(SS_DEFAULT_YEAR);

  /** Saat pertama buka layout SS: bulan default All Month (bukan bulan berjalan) */
  useEffect(() => {
    if (selectedChartModule !== "ss" || ssMonthDefaultApplied.current) return;
    ssFilter.setMonth("all");
    setSsBreakdownMonth("all");
    ssMonthDefaultApplied.current = true;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedChartModule]);
  const [siteOptions, setSiteOptions] = useState<string[]>([]);

  useEffect(() => {
    if (ssFilter.year !== String(ssDashboardYear)) {
      ssFilter.setYear(String(ssDashboardYear));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ssDashboardYear]);

  useEffect(() => {
    if (selectedChartModule !== "ss") return;
    const url = (API as any).SUGGESTION_DISTINCT_SITES;
    if (!url) {
      // Fallback: extract sites from mock data
      const mockSites = (DASHBOARD_DEMO_DATA.ssChart || []).map((r: any) => r.site).filter(Boolean);
      if (mockSites.length > 0) setSiteOptions(mockSites);
      return;
    }
    ...
    fetch(url, {
      ...
      .catch(() => {
        // Fallback: extract sites from mock data on network error
        const mockSites = (DASHBOARD_DEMO_DATA.ssChart || []).map((r: any) => r.site).filter(Boolean);
        if (mockSites.length > 0) setSiteOptions(mockSites);
      });
  }, [selectedChartModule, token]);
  ...
  const ssChartRequestBody = useMemo(
    () => ({
      ...
      year: ssDashboardYear,
      month:
        selectedSiteSS === "all"
          ? ssFilter.month === "all"
            ? 0
            : monthToNumber(ssFilter.month)
          : 0,
      siteName: selectedSiteSS === "all" ? "" : selectedSiteSS,
      pageNumber: 0,
      pageSize: 0,
    }),
    [
  ...
  const ssChartUrl =
    ssChartLevel === "site"
      ? (API as any).SUGGESTION_DASHBOARDCHART_SITE
      : (API as any).SUGGESTION_DASHBOARDCHART;

  const ssDash = usePostJson<any>({
    enabled: selectedChartModule === "ss" && !!ssChartUrl && orgMaster.ready,
    url: ssChartUrl,
    body: ssChartRequestBody,
    token,
    mapData: (raw) => raw, // Don't force array here
    onError: (status, payload) =>
      console.error("SS dashboard chart http error:", status, payload),
  });

  const ssMonthlyData = useMemo(() => {
    if (selectedSiteSS === "all") return [];

    const rawData = ssDash.data;
    const rows = Array.isArray(rawData) 
      ? rawData 
      : (rawData?.monthlyData || rawData?.MonthlyData || []);

    return rows.map((r: any) => ({
      department: r.Label || r.label || r.department || r.orgUnit || r.site || r.monthName || "-",
      Plan: Number(r.Plan ?? r.plan ?? r.target ?? 0),
      Actual: Number(r.Actual ?? r.actual ?? 0),
    }));
  }, [selectedSiteSS, ssDash.data]);

  const ssBreakdownRequestBody = useMemo(
    () => ({
      action: 1,
      division: ssFilter.division ?? "",
      department: ssFilter.department ?? "",
      ...
      year: ssDashboardYear,
      month:
        ssBreakdownMonth === "all" ? 0 : monthToNumber(ssBreakdownMonth),
      siteName: selectedSiteSS === "all" ? "" : selectedSiteSS,
      pageNumber: 0,
      pageSize: 0,
    }),
  ...
  const ssBreakdownDash = usePostJson<any>({
    enabled: selectedChartModule === "ss" && !!ssChartUrl && orgMaster.ready && selectedSiteSS !== "all",
    url: ssChartUrl,
    body: ssBreakdownRequestBody,
    token,
    mapData: (raw) => raw,
    onError: (status, payload) =>
      console.error("SS breakdown chart http error:", status, payload),
  });

  const ssBreakdownData = useMemo(() => {
    if (selectedSiteSS === "all") return [];

    const rawRows = (ssBreakdownDash.data?.BreakdownData || ssBreakdownDash.data?.breakdownData || (Array.isArray(ssBreakdownDash.data) ? ssBreakdownDash.data : []));
    return rawRows.map((r: any) => {
      const p = Number(r.Plan ?? r.plan ?? r.target ?? 0);
      const a = Number(r.Actual ?? r.actual ?? 0);
      return {
        department: r.Label || r.label || r.department || r.orgUnit || "-",
        Plan: p,
        Actual: a,
        Achievement: calcAchievement(p, a),
      };
    });
  }, [selectedSiteSS, ssBreakdownDash.data]);

  const ssAllMonthBreakdownData = useMemo(() => {
    if (selectedSiteSS === "all") return [];
    const rawData = ssDash.data;
    const rawRows = (rawData?.BreakdownData || rawData?.breakdownData || (Array.isArray(rawData) ? rawData : []));
    return rawRows.map((r: any) => {
      const p = Number(r.Plan ?? r.plan ?? r.target ?? 0);
      const a = Number(r.Actual ?? r.actual ?? 0);
      return {
  ...
  const ssDivisionsFromChartApi = useMemo(() => {
    const rawData = ssDash.data;
    const rows = Array.isArray(rawData) ? rawData : (rawData?.breakdownData || rawData?.BreakdownData || []);
    return uniqueLabelsFromRows(rows, "division");
  }, [ssDash.data]);
  ...
  const ssChartBound = useSsChartBound({
    rows: ssChartLevel === "org" ? (ssDash.data || []) : [],
    filter: ssFilter,
    mergedDivisions,
    mergedDepartments,
    mergedSections,
  ...
  const ssChartData = useMemo(() => {
    if (ssChartLevel === "site") {
      const rawData = ssDash.data;
      const rows = Array.isArray(rawData) 
        ? rawData 
        : (rawData?.breakdownData || rawData?.BreakdownData || []);

      return rows.map((r: any) => ({
        department: r.Label || r.label || r.division || r.orgUnit || r.site || "-",
        Plan: Number(r.Plan ?? r.plan ?? r.target ?? 0),
        Actual: Number(r.Actual ?? r.actual ?? 0),
      }));
    }
    return ssChartBound.data || [];
  }, [ssChartLevel, selectedSiteSS, ssDash.data, ssChartBound.data]);

  const ssMaxTotal = useMemo(() => {
    let m = 0;
    for (const r of ssChartData || []) {
      m = Math.max(m, Number(r.Plan || 0), Number(r.Actual || 0));
    }
    return m;
  }, [ssChartData]);
  ...
  const ssTotalAxis = useMemo(() => {
    const p = Number(ssTotalReq.data?.totalTarget || 0);
    const a = Number(ssTotalReq.data?.totalActual || 0);
    return buildNiceTicks(Math.max(p, a, 10) * 1.2, 4);
  }, [ssTotalReq.data]);
  const ssCollectedTotal = useMemo(
    () => (ssChartData || []).reduce((sum, row) => sum + Number(row.Actual || 0), 0),
    [ssChartData]
  );
  ...
  const ssClassificationRequestBody = useMemo(
    ...
    [ssFilter.division, ssFilter.department, ssDashboardYear, ssFilter.month]
  );
  ...
  const mon = useMonitoring({
    projects: [
      ...projects.filter((p) => p.type !== "QCC" && p.type !== "QCP"),
  ...
  return (
    <div className="w-full max-w-none py-6 sm:py-10">
      ...
      <Card className="shadow-md border border-border/50 bg-white rounded-2xl mb-8">
      ...
      {selectedChartModule === "ss" && (
        <div className="space-y-8 mt-0">
          <SsDashboardFilterBar year={ssDashboardYear} onYearChange={setSsDashboardYear} />

          <FinishOverviewBarChart
            title="SS Total"
            subtitle={`Plan vs Actual semua jobsite — tahun ${ssDashboardYear}`}
            target={Number(ssTotalReq.data?.totalTarget ?? 0)}
            actual={Number(ssTotalReq.data?.totalActual ?? 0)}
            targetColor={SS_CHART_PALETTE.plan}
            actualColor={SS_CHART_PALETTE.actual}
            iconAccentClass="bg-[#e88539]/10 text-[#e88539]"
            loading={ssTotalReq.loading}
          />

          <SsMonthlyAllSitesChart year={ssDashboardYear} loading={ssDash.loading} />

          {selectedSiteSS === "all" ? (
            <div className="grid grid-cols-1 gap-8">
              <PlanActualChartCard
                title="SS Collected (All Sites)"
                subtitle="Performance distribution across all operative jobsites"
                {...SS_CHART_COLORS}
                {...SS_CHART_BAR_SITES}
                siteValue={selectedSiteSS}
                onSiteChange={setSelectedSiteSS}
                siteOptions={siteOptions}
                data={ssChartData}
                year={String(ssDashboardYear)}
                month={ssFilter.month}
                onYear={() => {}}
                onMonth={ssFilter.setMonth}
                hideFilters={true}
                showMonthFilterOnly={true}
                showPaging={false}
                pageIndex={0}
                totalPages={1}
                onPrevPage={() => {}}
                ...
                yDomain={[0, ssAxis.domain[1]]}
                yTicks={ssAxis.ticks}
              />
            </div>
          ) : (
            /* 3. Site Specific View - Side by Side */
            <Card className="shadow-md border border-border/50 bg-white rounded-2xl overflow-hidden transition-all duration-300 mt-2">
              <CardHeader className="p-2 pb-2">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-primary/10">
                        <BarChart3 className="w-5 h-5" style={{ color: BRAND_SIDEBAR.teal }} />
                      </div>
                      <CardTitle className="text-xl sm:text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70">
                        Site Specific View
                      </CardTitle>
                    </div>
                    <p className="text-sm text-muted-foreground font-medium opacity-80 mt-1.5 ml-[44px]">
                      Detailed metrics for {selectedSiteSS}
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-[180px]">
                        <Select value={selectedSiteSS} onValueChange={setSelectedSiteSS}>
                          <SelectTrigger className="h-9 text-xs bg-muted/30 border-muted-foreground/10 hover:bg-muted/50 rounded-lg transition-colors">
                            <SelectValue placeholder="All Site" />
                          </SelectTrigger>
                          <SelectContent className="rounded-xl border-border/50">
                            <SelectItem value="all" className="rounded-md">All Site</SelectItem>
                            {siteOptions.map((s) => {
                              const label = s === "JAHO" ? "JAHO-NARO" : s;
                              return (
                                <SelectItem key={s} value={s} className="rounded-md">{label}</SelectItem>
                              );
                            })}
                      ...
                      <Select value={ssBreakdownMonth} onValueChange={setSsBreakdownMonth}>
                        <SelectTrigger className="w-[130px] h-9 text-xs font-semibold bg-muted/30 border-muted-foreground/10 rounded-lg">
                          <SelectValue placeholder="Month" />
                        </SelectTrigger>
                        <SelectContent>
              ...
              <CardContent className="p-0">
                <div className="grid grid-cols-2 items-stretch divide-x divide-border/30 border-t border-border/30">
                  {/* Left: Monthly Trend Chart */}
                  <div className="p-2">
                    <PlanActualChartCard
                      title="Monthly Trend"
                      subtitle={`${selectedSiteSS} Performance Trend - All Months`}
                      {...SS_CHART_COLORS}
                      {...SS_CHART_BAR_DENSE}
                      data={ssMonthlyData}
                      year={ssFilter.year}
                      ...
                      yDomain={[0, ssAxis.domain[1]]}
                      centeredTitle={false}
                      hideCardOutline={true}
                      chartHeight={450}
                    />
                  </div>

                  {/* Right: Breakdown Chart (Section/Department) */}
                  <div className="p-2">
                    <PlanActualChartCard
                      title={["MACO", "SERA"].includes(selectedSiteSS) ? `Sectional View` : `Department View`}
                      subtitle={`${selectedSiteSS} Distribution - All Months ${ssDashboardYear}`}
                      {...SS_CHART_COLORS}
                      {...SS_CHART_BAR_DENSE}
                      data={ssAllMonthBreakdownData}
                      year={ssFilter.year}
                      month="all"
                      onYear={ssFilter.setYear}
                      onMonth={() => {}}
                      ...
                      yDomain={[
                        0,
                        Math.max(
                          ...ssAllMonthBreakdownData.map((r) =>
                            Math.max(r.Plan || 0, r.Actual || 0)
                          ),
                          10
                        ) * 1.2,
                      ]}
                ...
                {ssBreakdownMonth !== "all" && (
                  <div className="border-t border-border/30 p-2">
                    <PlanActualChartCard
                      title={
                        ["MACO", "SERA"].includes(selectedSiteSS)
                          ? `Sectional View — Actual (${ssBreakdownMonth})`
                          : `Department View — Actual (${ssBreakdownMonth})`
                      }
                      subtitle={`${selectedSiteSS} — Actual per department, ${ssBreakdownMonth} ${ssDashboardYear}`}
                      {...SS_CHART_COLORS}
                      {...SS_CHART_BAR_DENSE}
                      data={ssBreakdownData}
                      year={String(ssDashboardYear)}
                      month={ssBreakdownMonth}
                      onYear={() => {}}
                      onMonth={() => {}}
                      hideFilters={true}
                      hideYear={true}
                      ...
                      onDepartment={() => {}}
                      onSection={() => {}}
                      yDomain={[0, Math.max(...ssBreakdownData.map((r) => r.Actual || 0), 10) * 1.2]}
                      centeredTitle={false}
                      hideCardOutline={true}
                      chartHeight={450}
                    />
              ...
              {mon.selectedModule && mon.selectedModule !== "qcc" && mon.selectedModule !== "qcp" && selectedModuleConfig && selectedTheme && (
                ...
                >
                  ...
                  {isQFamily(mon.selectedModule) && mon.selectedStatus === "in-progress" && (
                    <StepsPanel
                      moduleId={mon.selectedModule}
                      projects={mon.statusFilteredProjects}

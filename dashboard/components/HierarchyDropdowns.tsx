  ...
  return (
    <div className="relative inline-block text-left">
      ...
      {props.open && (
        <div
          className={cn(
            "absolute left-0 mt-2 z-[100] w-64 rounded-xl border bg-white text-popover-foreground shadow-2xl animate-in fade-in zoom-in-95 duration-200",
...
export function HierarchyDropdowns(props: {
  ...
  const siteItems = useMemo(() => (props.sites || []).map((x) => ({ value: x, label: x })), [props.sites]);
    ...
    <div ref={wrapRef} className="mt-2 flex flex-wrap gap-2">
      {props.onSite && (
        <DropdownFilter
          ...
          allLabel={props.siteAllLabel ?? "All Site"}
          selectedValue={props.siteValue || null}
          onSelect={props.onSite}
          items={siteItems}
